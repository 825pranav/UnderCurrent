...
\multicolumn{5}{l}{\footnotesize $^{b}$Python FSM gate is primary enforcer; this count predates wasmtime activation. WASM sandbox validated separately (all 20 policy cases correct, $<$1\,ms overhead).}
\end{tabular}
\end{table*}
...
Evaluation on a live \texttt{postgres} container confirmed $M_2 = M_6 = 0.3697$ (44 FSM-gated cycles of 119) and $M_4 = 1.0000$ across all dispatched actions, with schema validation checks $M_1 = M_5 = 1.0000$. That two independently defined metrics land on the same value is not accidental: cycles where FSM history changes the dispatched action are precisely the cycles where the safety gate fires. A subsequent fault-injection campaign across \texttt{postgres}, \texttt{redis}, and \texttt{mysql} (90 episodes, all recovered) confirmed the FSM two-cycle structural guarantee---MTTR is dominated by two reconcile intervals plus workload-specific restart latency, with within-workload $\sigma/\mu < 1\%$. The WASM policy sandbox (wasmtime~45.0.0) is active and validated: all four action modules produce correct policy decisions with sub-millisecond overhead. These results support the core design claim that FSM-gated sequencing prevents unsafe early execution of checkpoint-and-restart; broader workload coverage and real eBPF fault injection remain the main scope limitations.
