...
\item A six-metric evaluation framework with formal equations and results from live eBPF probes on real Docker containers, separating nominal-baseline from fault-injection findings (Section~\ref{sec:eval}).
\end{enumerate}
...
Table~\ref{tab:fsm} gives the full transition relation. The FSM persists across cycles and resets only when a complete recovery finishes. The key constraint is Degraded$\rightarrow$Audited: \texttt{checkpoint\_and\_restart} cannot fire until at least one \texttt{flush\_io\_queue} cycle has completed, enforcing an MTTR lower bound of one reconcile interval---5\,s by default.
...
$M_3$ (indicative only, $N=2$). Two complete fault episodes were observed for \texttt{postgres}: 61.0\,s and 424.6\,s, giving a mean of 242.8\,s. With only two episodes differing by a factor of seven, this mean is not statistically meaningful and is reported for completeness only. What the FSM guarantees structurally is a lower bound of one reconcile interval (5\,s) on time to the Audited state, through the mandatory Degraded$\rightarrow$Audited transition.
...
M3 (MTTR, seconds)     & ---    & ---    & 242.8  & $N = 2$ episodes$^{a}$ \\
...
\hline
\multicolumn{5}{l}{\footnotesize $^{a}$Episodes: 61.0\,s and 424.6\,s; mean $= 242.8$\,s; wider sampling is future work.}\\
\multicolumn{5}{l}{\footnotesize $^{b}$Policy layer in Python implementation mode; WASM sandbox is a configurable upgrade path.}
\end{tabular}
\end{table*}
...
A few things to be upfront about. Experiments ran on a developer workstation, not a Kubernetes cluster. The eBPF probe architecture requires Linux~4.14+, and specific attachment points may need updating on kernels that have renamed these symbols. The \texttt{checkpoint\_and\_restart} action logged the checkpoint intent but did not invoke CRIU, which requires \texttt{--privileged} mode; only \texttt{docker restart} was executed. Consequently, the FSM sequencing argument for checkpoint safety was exercised at the decision level but the underlying checkpoint operation was not validated. M3 is based on $N = 2$ fault episodes---61.0\,s and 424.6\,s---which is far too few for robust MTTR characterisation; a proper campaign with many independently sampled episodes is needed. The policy enforcement layer ran in Python mode the whole time; the WASM sandbox, which would add process-level isolation, has not been validated. Only \texttt{postgres} generated qualifying Type~F fault signals: \texttt{nginx} and \texttt{redis} produced no \texttt{vfs\_write\_error} or high-latency \texttt{blk\_io\_latency} events during the evaluation window, so the Type~F response to those container types remains untested. The Phase~1 baseline traces are not included in the committed repository; only the Phase~2 evaluation snapshot is available for independent verification. Workload classification is static at configuration time; runtime reclassification is not supported.
