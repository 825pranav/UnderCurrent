...
$M_3$ ($N = 90$, three workloads). The \texttt{scripts/run\_episodes.py} harness ran 30 independent fault episodes per workload (\texttt{postgres}, \texttt{redis}, \texttt{mysql}), injecting eight high-latency \texttt{blk\_io\_latency} events (50--54\,ms, well above the 10\,ms threshold) into a fresh \texttt{StatefulStateStore} each episode and measuring time from fault injection to \texttt{checkpoint\_and\_restart} execution. All 90 episodes recovered; none timed out. Results by workload: \texttt{postgres} $\mu = 10.87$\,s, $\sigma = 0.09$\,s; \texttt{redis} $\mu = 10.40$\,s, $\sigma = 0.03$\,s; \texttt{mysql} $\mu = 11.54$\,s, $\sigma = 0.10$\,s. MTTR differences reflect container-specific recovery latency: CRIU checkpoint creation succeeds on \texttt{postgres} and \texttt{redis} ($\approx$410\,ms and $\approx$20\,ms overhead respectively) before the netns restore limitation forces fallback to \texttt{docker restart}; on \texttt{mysql} the CRIU checkpoint itself fails (runc multi-process constraint), so the checkpoint phase is faster ($\approx$210\,ms) but mysql's server initialisation makes \texttt{docker restart} slower. The tight $\sigma/\mu$ across all three workloads ($< 1\%$) confirms the FSM structural guarantee: every episode followed the identical two-cycle sequence---\texttt{flush\_io\_queue} in cycle~1 (FSM: Healthy$\rightarrow$Degraded$\rightarrow$Audited), \texttt{checkpoint\_and\_restart} in cycle~2.
...
M3 (MTTR, seconds)     & ---    & ---    & 10.94  & $N = 90$ episodes (3 workloads), $\sigma = 0.55$\,s$^{a}$ \\
