...
\item A four-metric behavioural evaluation---divergence rate (M2), MTTR (M3), reversibility ratio (M4), and FSM-gated suppression rate (M6)---supplemented by two schema validation checks (M1, M5), with results from live eBPF probes on real Docker containers (Section~\ref{sec:eval}).
...
Type~S recorded 1,187 entries over the 32-minute baseline, 1,039 of them active decisions---all reversible ($M_4 = 1.0000$). Type~F logged 386 cycles for \texttt{postgres}; the FSM remained in Healthy throughout and \texttt{no\_action} was issued on every cycle. A stateful controller that fires spuriously on healthy workloads would contaminate MTTR measurements and render M2 uninterpretable; this baseline confirms neither happens. The 307 captured \texttt{blk\_io\_latency\_normal} events confirm that block-I/O telemetry was active with all latencies below the $\Lambda = 10{,}000\,\mu$s threshold throughout ($M_1 = M_2 = M_6 = 0.0000$).
...
$M_4 = 1.0000$. All 1,039 Type~S and 95 Type~F Phase~2 active decisions were reversible: \texttt{restart}, \texttt{reschedule}, \texttt{flush\_io\_queue}, or \texttt{checkpoint\_and\_restart}. The only action classified as irreversible in this configuration, \texttt{volume\_delete}, was never triggered by the observed fault patterns. M4\,=\,1.000 therefore confirms that the reversibility gate never needed to activate---the system operated entirely within the reversible action tier throughout these experiments.
...
$M_3$ (three workloads, 30 episodes each). The \texttt{scripts/run\_episodes.py} harness injected eight \texttt{blk\_io\_latency} events (50--54\,ms, above the 10\,ms threshold) directly into a fresh \texttt{StatefulStateStore}---bypassing eBPF---and measured wall time from fault injection to the first \texttt{checkpoint\_and\_restart} dispatch. In all 90 episodes the effective recovery action was \texttt{docker restart}: CRIU checkpoint creation succeeded on \texttt{postgres} ($\mu = 405$\,ms, $\sigma = 32$\,ms, $N=30$) and \texttt{redis} (per-episode latency not recorded) but restore via \texttt{docker start --checkpoint} failed on both due to a Linux~5.15 netns bind-mount constraint; on \texttt{mysql} CRIU checkpoint creation itself fails (multi-process runc constraint, $\mu = 232$\,ms to failure, $N=30$) so the fallback fires immediately. Results (Fig.~\ref{fig:mttr}): \texttt{postgres} $\mu = 10.87$\,s, $\sigma = 0.09$\,s; \texttt{redis} $\mu = 10.40$\,s, $\sigma = 0.03$\,s; \texttt{mysql} $\mu = 11.54$\,s, $\sigma = 0.10$\,s. The cross-workload spread (1.14\,s) reflects \texttt{docker restart} startup time: redis restarts fastest (single-process, minimal initialisation), mysql is slowest (server init and InnoDB recovery). The within-workload $\sigma/\mu < 1\%$ is a structural property, not a statistical one: because the FSM is deterministic given fixed fault severity and a fixed reconcile interval, every episode follows the identical two-cycle sequence---\texttt{flush\_io\_queue} in cycle~1 (FSM: Healthy$\rightarrow$Degraded$\rightarrow$Audited), \texttt{checkpoint\_and\_restart} dispatched in cycle~2---and MTTR variation within a workload reduces to the variance of \texttt{docker restart} itself.
...
\begin{axis}[
  ...
  width=0.90\columnwidth, height=4.8cm,
...
\end{axis}
\end{tikzpicture}
\caption{Mean MTTR per workload, $N=30$ episodes each. Error bars show $\pm1\sigma$ (within-workload). Y-axis starts at 10\,s; value labels give exact means. The workload spread reflects \texttt{docker restart} startup time, not CRIU or FSM differences---all three workloads follow the same two-cycle decision sequence.}
\label{fig:mttr}
\end{figure}

\begin{table*}[!t]
\caption{METRIC VALUES. M1--M2, M4--M6: REAL-SYSTEM EBPF EVALUATION (PHASE~1: $N_S = 1{,}187$, $N_F = 386$; PHASE~2: $N_F = 119$). M3: SYNTHETIC FAULT INJECTION HARNESS, $N = 90$ EPISODES (3 WORKLOADS).}
\label{tab:metrics}
\centering
\renewcommand{\arraystretch}{1.15}
...
M3 (MTTR, seconds)     & ---    & ---    & 10.87 / 10.40 / 11.54  & postgres / redis / mysql, $N=30$ each$^{a}$ \\
...
\hline
\multicolumn{5}{l}{\footnotesize $^{a}$90/90 episodes recovered (0 timeouts). Per-workload: postgres $10.87 \pm 0.09$\,s; redis $10.40 \pm 0.03$\,s; mysql $11.54 \pm 0.10$\,s. Every episode: flush cycle 1, C\&R cycle 2.}\\
...
All nine scenarios in Table~\ref{tab:scenarios} produced the expected action (accuracy: 1.00). The equality $M_2 = M_6 = 0.3697$ is not coincidental. Both metrics count the same set of cycles from different vantage points: a cycle where the real FSM's accumulated state changes the dispatched action (M2) is, by construction, a cycle where the safety gate overrides the raw DAG output (M6). The two metrics are definitionally linked under the Type~F architecture. Whether this equality holds under other fault patterns is a property of the scenario distribution, not a general guarantee, and should not be assumed.

\subsection{Baseline Comparison and Ablation}
\label{sec:ablation}

To answer ``why is the FSM necessary?'', we replayed the $N = 2{,}282$ real stateful trace entries from Phase~2 under three counterfactual decision policies, holding the recorded \texttt{score} values constant while varying only the decision mechanism (Table~\ref{tab:ablation}).

\begin{table}[t]
\caption{Decision-policy ablation on $N = 2{,}282$ real Type-F traces. Input confidence scores are held constant; only the decision mechanism varies. \emph{Unsafe C\&R}: dispatched from a pre-audit FSM state. \emph{Missed C\&R}: legitimate checkpoint opportunity suppressed by a memoryless or binary policy.}
\label{tab:ablation}
\centering
\begin{tabular}{lrcc}
\hline
...
\textbf{Threshold-only (shadow path).} A stateless threshold policy---equivalent to instantiating a fresh FSM every cycle---can never reach Audited within a single cycle and therefore never approves C\&R. This variant misses all 44 legitimate C\&R opportunities. Notably, this is the same computation already performed by \texttt{reconcile\_both()} in every cycle: $M_2 = 0.3697$ (44 divergent cycles of 119 active fault-injection cycles) quantifies exactly how often the shadow path would have made the wrong call.

\textbf{No-confidence.} Replacing the continuous score with a binary fault/no-fault signal (score $\in \{0, 0.75\}$) also yields 0 C\&R dispatches, because $0.75 < \theta_{\mathrm{repair}} = 0.80$: every fault cycle is downgraded to \texttt{flush\_io\_queue}. Graduated confidence scoring is necessary for C\&R to be reachable at all.
...
Three hyperparameters were swept independently, holding all others at nominal values. For each setting, the controller was evaluated over its full fault scenario set. Table~\ref{tab:sensitivity} reports macro-averaged F1 across scenarios.

\begin{table}[t]
\caption{THRESHOLD SENSITIVITY (MACRO-AVERAGED F1 OVER FAULT SCENARIOS; EACH PARAMETER SWEPT INDEPENDENTLY).}
...
\textbf{window\_seconds} is equally stable: F1\,=\,0.833 from 15\,s to 240\,s. Two fault scenarios (cpu\_flicker, ambiguous) are inherently not representable by either scorer's signal classes and remain undetectable at any window length; all other scenarios are consistently detected. The count-based scoring model accumulates evidence proportionally to window size, making it insensitive to window length above the minimum needed to observe a single event.
...
Two sources of per-cycle overhead are evaluated (Table~\ref{tab:overhead}).

\begin{table}[t]
\caption{Runtime overhead (N\,=\,10{,}000 calls each, post JIT warmup). Both costs are negligible relative to the 5\,s reconcile interval.}
\label{tab:overhead}
\centering
\begin{tabular}{lrrrr}
\hline
...
\textbf{WASM policy sandbox.} Each \texttt{policy\_check()} call creates a fresh \texttt{Store} and \texttt{Instance} from a pre-compiled cached module; compilation is amortised at process startup. Across 10{,}000 post-warmup calls (2{,}500 per action module) the mean is 27\,$\mu$s (stddev 12\,$\mu$s, p95 30\,$\mu$s, p99 50\,$\mu$s). An equivalent pure-Python FSM state lookup costs 0.12\,$\mu$s (p50); the sandbox adds $230{\times}$ the raw decision cost. In absolute terms 27\,$\mu$s is 0.0005\,\% of the 5\,s reconcile interval, making the overhead inconsequential for the target operating cadence.

\textbf{Shadow execution.} Running \texttt{reconcile\_both()} (real FSM + throwaway shadow FSM) takes 14.9\,$\mu$s versus 6.9\,$\mu$s for a single \texttt{reconcile()} call---an 8\,$\mu$s absolute overhead, 117\,\% of single-path cost. The proportional figure is large because both paths perform equivalent decision work; the absolute cost is negligible because the control loop sleeps 5\,s between iterations. This confirms that shadow execution is a practical instrument for production deployment, not a research-only technique.
...
\textbf{Type~S has no FSM} because stateless workloads carry no on-disk state that an untimely restart can corrupt; audit-phase sequencing would add one full reconcile interval of latency with no corresponding safety benefit. Shadow computation on the Type~S track is therefore always identical to the real path---real and shadow actions agree on every cycle by construction---so M2 is informative only on the Type~F track, where accumulated FSM state causes the paths to diverge. Shadow execution overhead is 8\,$\mu$s per cycle (117\,\% of single-path cost; 0.0002\,\% of the 5\,s interval), negligible in absolute terms.

\subsection{Limitations}
\label{sec:limitations}

\textbf{Platform scope.} Experiments ran on a developer workstation running Linux~5.15, not a Kubernetes cluster. The eBPF probe architecture requires Linux~4.14+; specific kprobe attachment points may need updating on kernels that have renamed the relevant symbols. Workload classification is static at configuration time; runtime reclassification is not supported.

\textbf{CRIU and checkpoint restore.} CRIU checkpoint creation (\texttt{docker checkpoint create}) was validated on \texttt{postgres} and \texttt{redis}: the checkpoint artefact is produced with exit code~0. Restore via \texttt{docker start --checkpoint} fails on this host due to a network-namespace bind-mount limitation in Linux~5.15; the restored process cannot re-attach to its original netns, so recovery falls back to \texttt{docker restart}. The FSM sequencing guarantee---that \texttt{checkpoint\_and\_restart} cannot fire before the audit phase completes---is fully exercised at the decision layer; the checkpoint artefact is created; only the in-place memory restore is constrained by the kernel. On \texttt{mysql}, CRIU checkpoint creation itself fails due to a multi-process runc constraint ($\mu = 232$\,ms to failure, $N=30$), so the \texttt{docker restart} fallback fires immediately.

\textbf{Fault injection and workload coverage.} M3 is based on $N = 90$ synthetic fault episodes ($N = 30$ per workload, all recovered). Fault injection inserts \texttt{blk\_io\_latency} events directly into a fresh \texttt{StatefulStateStore}, bypassing eBPF; real VFS fault injection via \texttt{/dev/full} or \texttt{stress-ng} would connect detection and recovery into a single observable latency. The eBPF-driven real-event path was validated on \texttt{postgres} in Phase~2 only; \texttt{mysql} and \texttt{redis} fault injection was synthetic. The Phase~1 baseline traces are not committed to the repository; only the Phase~2 snapshot is available for independent verification.

\textbf{WASM policy sandbox.} The sandbox (\texttt{wasmtime} 45.0.0) is validated: all four action modules compile and execute, \texttt{policy\_check()} returns the correct verdict in all 20 test cases, and steady-state overhead is 27\,$\mu$s mean (50\,$\mu$s at p99, $N = 10{,}000$ post-warmup). The M3 fault campaign and ablation trace corpus predate wasmtime activation; M6 counts therefore reflect Python-gate suppressions only.

\subsection{Future Work}

Deploying on a real Kubernetes cluster is the obvious next step---the per-host eBPF model would be replaced by a DaemonSet-based probe agent, enabling evaluation across multi-node workloads. CRIU restore via \texttt{docker start --checkpoint} currently fails on Linux~5.15 due to a netns bind-mount limitation; testing on a newer kernel or with rootful containers is a concrete near-term step. A fault-injection harness using real VFS errors (\texttt{/dev/full}), \texttt{tc netem}, or \texttt{stress-ng} would connect M3 measurement to eBPF-captured events and make the end-to-end latency (detection + decision + recovery) observable. Formal TLA+ verification of the FSM transition table in Table~\ref{tab:fsm} is planned.
...
Evaluation on a live \texttt{postgres} container confirmed $M_2 = M_6 = 0.3697$ (44 FSM-gated cycles of 119) and $M_4 = 1.0000$ across all dispatched actions, with schema validation checks $M_1 = M_5 = 1.0000$. That two independently defined metrics land on the same value is not accidental: cycles where FSM history changes the dispatched action are precisely the cycles where the safety gate fires. A fault-injection campaign across \texttt{postgres}, \texttt{redis}, and \texttt{mysql} ($N = 90$ episodes, all recovered) confirmed the FSM two-cycle structural guarantee---MTTR ranges 10.40--11.54\,s across workloads, within-workload $\sigma/\mu < 1\%$. An ablation study on 2,282 real Type-F traces directly answers ``why is the FSM necessary?'': without FSM gating, 44 of 88 C\&R dispatches are premature (fired before the audit phase); without state accumulation (memoryless threshold), all 44 legitimate C\&R opportunities are missed; the full FSM produces exactly 44 C\&R dispatches, all from Audited state. The WASM policy sandbox (wasmtime~45.0.0) adds 27\,$\mu$s per action call (p99: 50\,$\mu$s); shadow execution adds 8\,$\mu$s per cycle---both negligible at the 5\,s reconcile interval. These results support the core design claim that FSM-gated sequencing prevents unsafe early checkpoint-and-restart execution while remaining practical; broader workload coverage and real eBPF fault injection remain the main scope limitations.
