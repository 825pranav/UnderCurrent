...
M1, M2, M4--M6 derive from real-system eBPF trace data. M3 uses a synthetic fault-injection harness (§\ref{sec:m3}).
...
\textbf{WASM policy sandbox.} Each \texttt{policy\_check()} call creates a fresh \texttt{Store} and \texttt{Instance} from a pre-compiled cached module (compilation is amortised at startup). Across 10{,}000 calls this costs $27\,\mu$s mean, $50\,\mu$s at p99. An equivalent pure-Python FSM state lookup costs $0.12\,\mu$s; the sandbox adds 230$\times$ the raw decision cost, but $27\,\mu$s represents 0.0005\,\% of the 5-second reconcile interval, so the overhead is practically inconsequential.

\textbf{Shadow execution.} Running \texttt{reconcile\_both()} (real FSM + throwaway shadow FSM) takes 14.9\,$\mu$s versus 6.9\,$\mu$s for a single \texttt{reconcile()} call---an 8\,$\mu$s absolute overhead. The proportional cost is high (117\,\%) because both paths do equivalent decision work; the absolute cost is negligible because the reconcile loop sleeps 5\,s between iterations.
