...
$M_3$ (indicative only, $N=2$). Two complete fault episodes were observed for \texttt{postgres}: 61.0\,s and 424.6\,s, giving a mean of 242.8\,s. With only two episodes differing by a factor of seven, this mean is not statistically meaningful and is reported for completeness only. What the FSM guarantees structurally is a minimum delay of one reconcile interval (5\,s) between first fault detection and \texttt{checkpoint\_and\_restart} execution: Audited state is reached within the first fault cycle, but C\&R cannot execute until the following cycle. The minimum MTTR---from first active decision to first subsequent \texttt{no\_action}---is therefore at least two reconcile intervals (10\,s).
