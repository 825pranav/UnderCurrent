...
\hline
\multicolumn{4}{l}{\footnotesize $^{c}$An additional 7 disagreements exist: cycles where the full FSM produces \texttt{no\_action}} \\
\multicolumn{4}{l}{\footnotesize but no-confidence dispatches \texttt{flush\_io\_queue} (binary score 0.75 $\geq\tau_{\mathrm{flush}}=0.50$).} \\
\multicolumn{4}{l}{\footnotesize Total no-confidence disagreements vs.\ full FSM: 51 (\texttt{stateful/ablation\_results.json}).} \\
\end{tabular}
\end{table}
