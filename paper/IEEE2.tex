...
$M_4 = 1.0000$. All 95 active Type-F decisions were reversible: 51 \texttt{flush\_io\_queue} and 44 \texttt{checkpoint\_and\_restart}, both tagged as reversible in the trace schema. The \texttt{volume\_delete} action, which would be irreversible, was never triggered by the observed fault patterns. M4\,=\,1.0000 therefore confirms that the reversibility gate never needed to activate during this evaluation---the system operated entirely within the reversible action tier.
...
The equality $M_2 = M_6 = 0.0193$ is not coincidental. Both metrics count the same 44 cycles from different vantage points: a cycle where the real FSM's accumulated state changes the dispatched action (M2) is, by construction, a cycle where the safety gate overrides the raw DAG output (M6). The two metrics are definitionally linked under the Type~F architecture.
...
\end{axis}
\end{tikzpicture}
\caption{Mean MTTR for \texttt{redis}, $N = 30$ episodes (all recovered). Error bars show $\pm 1\sigma$. $\sigma/\mu = 0.25\%$; the near-zero variance is structural: every episode follows the identical two-cycle FSM sequence with MTTR variation reduced to the variance of \texttt{docker restart} startup time. Episode data for \texttt{postgres} and \texttt{mysql} was not committed and is not available.}
\label{fig:mttr}
\end{figure}
...
\begin{table}[t]
\caption{Runtime overhead ($N = 10{,}000$ calls each, post JIT warmup; \texttt{results/overhead\_benchmark.json}). Both costs are negligible relative to the 5\,s reconcile interval.}
\label{tab:overhead}
\centering
\begin{tabular}{lrrrr}
\hline
