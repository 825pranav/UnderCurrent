...
\node[font=\tiny\bfseries\itshape] at (-1.55, 3.94) {Type~S~~Stateless};
\node[font=\tiny\bfseries\itshape] at ( 1.55, 3.94) {Type~F~~Stateful};
...
\end{tikzpicture}
\vspace{2pt}
\caption{Dual-track control plane. Type~S (left) and Type~F (right) share the eBPF event bus but run independent scorers and decision engines. Type~F adds an FSM-gated DAG and a WASM policy enforcement layer between the DAG and the action executor. Every reconcile cycle runs on both a real path and a fresh-FSM shadow path; divergences increment metric~M2.}
\label{fig:arch}
\end{figure}
...
Results across three workloads ($N = 30$ episodes each, 90 total; Fig.~\ref{fig:mttr}): \texttt{postgres} $\mu = 10.87$\,s, $\sigma = 0.09$\,s; \texttt{redis} $\mu = 10.40$\,s, $\sigma = 0.03$\,s; \texttt{mysql} $\mu = 11.54$\,s, $\sigma = 0.10$\,s. In all 90 episodes the effective recovery action was \texttt{docker restart}: CRIU checkpoint creation succeeded on \texttt{postgres} and \texttt{redis} but restore via \texttt{docker start --checkpoint} fails on both due to a Linux~5.15 netns bind-mount constraint; on \texttt{mysql} CRIU checkpoint creation itself fails (multi-process runc constraint) so the fallback fires immediately. The cross-workload spread (1.14\,s) reflects \texttt{docker restart} startup time---redis is fastest (single-process, minimal initialisation), mysql is slowest (InnoDB recovery). Within-workload $\sigma/\mu < 1\%$ is structural: the FSM is deterministic at fixed fault severity and reconcile interval, so MTTR variation reduces to \texttt{docker restart} scheduling jitter.
...
\begin{axis}[
  ...
  every node near coord/.append style={font=\tiny, text=black,
    ...
    /pgf/number format/zerofill=true, yshift=10pt},
  nodes near coords,
...
\end{axis}
\end{tikzpicture}
\caption{Mean MTTR per workload, $N = 30$ episodes each (all recovered). Upper error bars show $1\sigma$ (within-workload). Y-axis starts at 10\,s; value labels give exact means. The cross-workload spread (1.14\,s) reflects \texttt{docker restart} startup time---all three workloads follow the same two-cycle FSM sequence. Within-workload $\sigma/\mu < 1\%$ is structural: FSM determinism reduces MTTR variance to \texttt{docker restart} scheduling jitter.}
\label{fig:mttr}
\end{figure}

\begin{table*}[!t]
\caption{METRIC VALUES. M1--M2, M4--M6: DERIVED FROM THE COMMITTED \texttt{data/phase2/} SNAPSHOT ($N = 9{,}132$ TOTAL TRACES; $N_{\mathrm{real}}^{\mathrm{F}} = 2{,}282$, $N_{\mathrm{real}}^{\mathrm{S}} = 2{,}284$). M3: SYNTHETIC FAULT INJECTION HARNESS ($N = 90$ EPISODES, 3 WORKLOADS).}
\label{tab:metrics}
\centering
...
M3 (MTTR, seconds)      & ---    & ---    & ---    & pg 10.87\,s, redis 10.40\,s, mysql 11.54\,s; $N=30$ each ($^{a}$) \\
...
\hline
\multicolumn{5}{l}{\footnotesize $^{a}$All 90 episodes recovered (0 timeouts). Two-cycle sequence confirmed in all episodes. Per-workload $\sigma$: postgres 0.09\,s, redis 0.03\,s, mysql 0.10\,s. CRIU restore falls back to \texttt{docker restart} on Linux~5.15.}\\
...
\textbf{CRIU and checkpoint restore.} CRIU checkpoint creation succeeded on \texttt{postgres} ($\mu = 405$\,ms, $\sigma = 32$\,ms, $N=30$) and \texttt{redis}; restore via \texttt{docker start --checkpoint} fails on both due to a Linux~5.15 network-namespace bind-mount constraint, so recovery falls back to \texttt{docker restart}. On \texttt{mysql} CRIU checkpoint creation itself fails (multi-process runc constraint), and the fallback fires immediately. The FSM sequencing guarantee---that \texttt{checkpoint\_and\_restart} cannot fire before the audit phase completes---is fully exercised at the decision layer regardless of the underlying checkpoint/restore outcome.

\textbf{Fault injection and workload coverage.} $M_3$ is based on $N = 90$ synthetic fault episodes across three workloads (30 each; all recovered). Fault injection inserts \texttt{blk\_io\_latency} events directly into a fresh \texttt{StatefulStateStore}, bypassing eBPF; real VFS fault injection via \texttt{/dev/full} or \texttt{stress-ng} would connect detection and recovery into a single observable end-to-end latency. The two undetectable fault scenarios (\texttt{cpu\_flicker}, \texttt{ambiguous}) indicate a representational gap: neither scorer's signal classes capture CPU-frequency transients or ambiguous co-occurrence patterns.
