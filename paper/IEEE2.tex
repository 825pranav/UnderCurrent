...
\node[wide, minimum height=0.72cm] (ebpf) at (0, 3.40)
  {\textbf{Kernel eBPF Probes}\\
   \texttt{process\_exit} $\cdot$ \texttt{tcp\_connect} $\cdot$ \texttt{blk\_io} $\cdot$ \texttt{vfs}};

% ---- perf buffer ----
\node[wide] (buf) at (0, 2.58) {perf ring buffer (per track)};
\draw[arr] (ebpf) -- (buf);

% ---- Track column headers (outside arrow paths) ----
\node[font=\tiny\bfseries\itshape] at (-1.55, 2.14) {Type~S~~Stateless};
\node[font=\tiny\bfseries\itshape] at ( 1.55, 2.14) {Type~F~~Stateful};

% ---- Two tracks side by side ----
\node[sbox] (esS)  at (-1.55, 1.78) {EventListener~+~filter};
\node[sbox] (esF)  at ( 1.55, 1.78) {EventListener~+~filter};
\node[sbox] (ssS)  at (-1.55, 1.24) {StateStore (60\,s)};
\node[sbox] (ssF)  at ( 1.55, 1.24) {StatefulStateStore (60\,s)};
\node[sbox] (scS)  at (-1.55, 0.70) {Confidence Scorer~S};
\node[sbox] (scF)  at ( 1.55, 0.70) {Confidence Scorer~F};
\node[sbox] (dagS) at (-1.55, 0.16) {Threshold DAG};
\node[sbox] (dagF) at ( 1.55, 0.16) {FSM-Gated DAG};
\node[box]  (pol)  at ( 1.55,-0.38) {WASM Policy Layer};
...
\node[wide, minimum height=0.64cm] (exec) at (0,-1.08)
  {Action Executor\\
...
\draw[dashed, gray!60] (0, 2.30) -- (0, -0.68);
\end{tikzpicture}
\vspace{2pt}
