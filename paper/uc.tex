...
The most natural next steps are Kubernetes deployment (DaemonSet-based probe agents, multi-node evaluation), real VFS fault injection to link M3 to the full detection-to-recovery path, and formal verification of the FSM transition relation in Table~\ref{tab:fsm}. TLA+~\cite{b9} can express the safety properties, and a framework like Anvil~\cite{b13} could extend this to liveness proofs for the full stateful control path. In a Kubernetes setting (Fig.~\ref{fig:k8s}), each node runs a privileged DaemonSet pod that loads the eBPF probes and maintains local FSM instances, avoiding cross-node coordination in the reconcile loop. Workload classification shifts from the static \texttt{comm} allow-list to pod labels assigned by a mutating admission webhook, and \texttt{docker restart} is replaced by pod deletion through the owning controller. Each node reports its divergence records centrally, which turns M2 into a cluster-wide control-plane health signal rather than a per-host one. None of this requires the architecture in Fig.~\ref{fig:arch} to change, since the FSM instances on each node already run independently of one another.
